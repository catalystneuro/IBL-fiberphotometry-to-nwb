...
NWB conversion scripts for IBL-fiberphotometry data to the
...
https://github.com/catalystneuro/IBL-fiberphotometry-to-nwb//tree/main/src/fiber_photometry/convert_session.py
...
conda activate IBL-fiberphotometry-to-nwb_env
```
...
```bash
git clone https://github.com/your_github_username/IBL-fiberphotometry-to-nwb
```
...
For example, in this conversion, the `FiberPhotometryBehaviorInterface` contains the code that converts all of the behavioral data to NWB from raw <FILE_TYPE> files.
...
For example, for the conversion `fiber_photometry` you can find a directory located in `src/IBL-fiberphotometry-to-nwb/fiber_photometry`. 
Inside each conversion directory you can find the following files:


* `convert_session.py`: this script defines the function to convert one full session of the conversion. 
